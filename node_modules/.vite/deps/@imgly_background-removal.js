...
var getOrt = async (useWebGPU) => {
	...
	if (useWebGPU) ort = (await import("./ort.webgpu.bundle.min-D9bNs4Rz.js")).default;
	else ort = (await import("./ort.bundle.min-11RTKgWe.js")).default;
	return ort;
};
...
async function removeForeground(image, configuration) {
	const { config, session } = await init(configuration);
	const [alphamask2, imageInput] = await runInference(await imageSourceToImageData(image, config), config, session);
	const outImageTensor = imageInput;
	const [width, height, channels] = outImageTensor.shape;
	const stride = width * height;
	for (let i = 0; i < stride; i += 1) outImageTensor.data[4 * i + 3] = 255 - alphamask2.data[i];
	return await imageEncode(outImageTensor, config.output.quality, config.output.format);
}
...
async function applySegmentationMask(image, mask, config) {
	...
	return await imageEncode(imageTensor, config.output.quality, config.output.format);
}
...
*/
//#endregion
export { alphamask, applySegmentationMask, preload, removeBackground, removeForeground, segmentForeground };
